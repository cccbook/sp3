...
void riscvInit() {
  mapNew(&opMap, LEN(opList)*3+1);
  for (int i=Sll; i<LEN(opList); i++) {
    Op *op = &opList[i];
    ASSERT(op->id == i, "riscvInit(): Error:id(%d)!=i(%d)\n", op->id, i);
    mapAdd(&opMap, op->name, op);
  }
  mapNew(&rMap, LEN(rList)*3+1);
  for (int i=0; i<LEN(rList); i++) {
    Ki *rp = &rList[i];
    mapAdd(&rMap, rp->k, rp);
  }
}
