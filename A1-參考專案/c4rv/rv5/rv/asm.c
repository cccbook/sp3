...
void pass1(char *inFile) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (*label) {
      ...
      char *lab = stAdd(&strTable, label);
      printf("lab=%s\n", lab);
      mapAdd(&symMap, lab, (void*)pc);
    }
    pc += size;
  }
  fclose(fp);
...
int32_t arg2i(char *arg) {
  ...
  if (strchr("0123456789-", arg[0])!=NULL) {
    ...
    return (int) mapGet(&symMap, arg);
  }
}

uint32_t gen() {
  Op *o = (Op*) mapGet(&opMap, p0);
...
void assemble(char *file) {
  ...
  riscvInit();
  mapNew(&symMap, SYM_MAX*3+1);
  stNew(&strTable, stab, TMAX);
  mapDump(&symMap);
  pass1(inFile);
  mapDump(&symMap);
